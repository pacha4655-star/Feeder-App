...
export const WelcomeScreen: React.FC = () => {
  ...
  return (
    <div className="w-full min-h-[100dvh] relative overflow-x-hidden overflow-y-auto bg-[#EBF3ED] dark:bg-slate-950 font-sans select-none transition-colors duration-200 flex flex-col justify-between">
      
      {/* Centered Responsive Composition Container (Matches Reference Exactly) */}
      <div className="w-full max-w-md sm:max-w-xl md:max-w-2xl lg:max-w-3xl mx-auto min-h-[100dvh] flex flex-col justify-between relative z-10">
        ...
        <div className="w-full flex flex-col flex-1 relative z-0">
          ...
          {/* Top Bar with Status-Safe Area & Language Selector at Top-Right */}
          <div className="pt-[max(0.6rem,env(safe-area-inset-top,0px))] px-4 flex items-center justify-between relative">
            <div className="w-20" /> {/* Spacer to balance language selector */}
            ...
            <div className="w-20 flex justify-end relative">
              <button
                ...
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-white/85 dark:bg-slate-800/85 backdrop-blur-xs border border-slate-200/80 dark:border-slate-700/80 text-[11px] font-semibold text-slate-700 dark:text-slate-200 shadow-2xs hover:bg-white dark:hover:bg-slate-800 transition-colors cursor-pointer"
          ...
          <div className={`text-center px-4 my-1 sm:my-1.5 ${animateEntrance ? 'anim-feeder-tagline' : ''}`}>
            <h1 className="text-sm xs:text-base sm:text-lg md:text-xl font-extrabold text-[#1E4D2B] dark:text-green-300 tracking-tight leading-tight font-['Outfit',sans-serif]">
              A kinder world for every animal.
            </h1>
            <p className="text-[10px] xs:text-[11px] sm:text-xs font-semibold text-[#1E4D2B]/85 dark:text-green-300/85 tracking-wide mt-0.5">
              Connect • Care • Protect • Empower
            </p>
          </div>

          {/* Hero Animal Image (Focal point 34.5% preserves all animals from parrot to duck) */}
          <div className={`w-full flex-1 min-h-[120px] max-h-[28vh] sm:max-h-[34vh] md:max-h-[38vh] overflow-hidden relative select-none ${animateEntrance ? 'anim-feeder-hero' : ''}`}>
            <img
              src="/assets/feeder-login-reference.png"
              alt="Feeder Animal Community"
        ...
        {/* =================================================================== */}
        <div className={`w-full bg-white dark:bg-slate-900 rounded-t-[36px] sm:rounded-t-[44px] shadow-[0_-12px_40px_rgba(0,0,0,0.08)] dark:shadow-[0_-12px_40px_rgba(0,0,0,0.5)] -mt-4 relative z-10 px-4 xs:px-5 sm:px-8 md:px-12 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom,0px))] flex flex-col justify-between flex-shrink-0 ${animateEntrance ? 'anim-feeder-panel' : ''}`}>
          
          {/* Feature Icon Row (6 features matching reference in exact order) */}
          <div className="grid grid-cols-6 gap-1 sm:gap-2 text-center pb-2 mb-1.5 border-b border-slate-100 dark:border-slate-800 select-none max-w-xl mx-auto w-full">
            {/* 1. Find Help */}
