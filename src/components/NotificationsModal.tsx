...
import { ArrowLeft, Bell, Check, AlertCircle, Heart, MessageSquare, X } from 'lucide-react';
...
export const NotificationsModal: React.FC = () => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'feeding_logged':
      case 'feeding':
        return <Heart className="w-4 h-4 text-green-700" />;
      case 'comment':
      case 'like':
  ...
  return (
    ...
    >
      ...
      >
        {/* Top Header */}
        <div className="sticky top-0 z-10 bg-white px-5 py-4 flex items-center justify-between border-b border-slate-100">
          <div className="flex items-center gap-2">
            <span className="w-8 h-8 rounded-xl bg-green-50 text-green-700 flex items-center justify-center">
              <Bell className="w-4 h-4" />
            </span>
            <h3 className="font-bold text-sm sm:text-base text-slate-800">Notifications</h3>
          </div>
          ...
          {notifications.map(n => (
            <div
              ...
              className={`p-4 flex items-start gap-3.5 cursor-pointer transition-colors ${
                !n.isRead ? 'bg-green-50/40 hover:bg-green-50/70' : 'bg-white hover:bg-slate-50'
              }`}
            >
              <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
                n.type === 'urgent_help' || n.type === 'urgent_nearby' ? 'bg-red-100' : 'bg-green-100'
              }`}>
                {getIcon(n.type)}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <h4 className={`text-xs sm:text-sm ${!n.isRead ? 'font-bold text-slate-900' : 'font-medium text-slate-700'}`}>
                    {n.title}
                  </h4>
