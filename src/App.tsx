    ...
    <aside className="w-80 xl:w-96 flex-shrink-0 hidden lg:flex flex-col gap-4 sticky top-20 self-start font-sans">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 border border-slate-200/80 dark:border-slate-800 shadow-2xs">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[10px] font-bold text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-950/60 px-2 py-0.5 rounded-full border border-green-200 dark:border-green-800">
            Active Hub
          </span>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 border border-slate-200/80 dark:border-slate-800 shadow-2xs">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            className="text-[11px] font-bold text-red-600 hover:text-red-700 dark:text-red-400 flex items-center gap-0.5"
          >
            ...
            urgentAlerts.map(alert => {
              ...
              return (
                <div
                  ...
                  className="p-2.5 rounded-xl border border-red-100 dark:border-red-900/60 bg-red-50/40 dark:bg-red-950/30 hover:bg-red-50/80 dark:hover:bg-red-950/60 transition-all cursor-pointer group"
                >
                  <div className="flex items-start gap-2.5">
                    <img
                      src={photoUrl}
                      alt={alert.title || 'Urgent Help'}
                      className="w-11 h-11 rounded-lg object-cover flex-shrink-0 border border-red-200 dark:border-red-900"
                      referrerPolicy="no-referrer"
                    />
                    <div className="min-w-0 flex-1">
                      <h4 className="text-xs font-bold text-slate-900 dark:text-white truncate group-hover:text-red-700 dark:group-hover:text-red-400 transition-colors">
                        {alert.title}
                      </h4>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate mt-0.5">{alert.location || 'Local area'}</p>
                      <div className="flex items-center justify-between mt-1">
                        <span className="text-[10px] font-semibold text-red-600 dark:text-red-400">
                          {respondersCount} on the way
                        </span>
                        <span className="text-[10px] font-bold text-red-700 dark:text-red-300 bg-white dark:bg-slate-800 px-2 py-0.5 rounded-full border border-red-200 dark:border-red-800 flex items-center gap-0.5">
        ...
        <div className="space-y-2.5">
          {suggestedPeople.map(person => (
            <div
              ...
              className="flex items-center justify-between p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer group"
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-1">
                    <span className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate group-hover:text-green-700 dark:group-hover:text-green-400">
                      {person.name}
                    </span>
              ...
              <ChevronRight className="w-3.5 h-3.5 text-slate-300 dark:text-slate-600 group-hover:text-slate-600 dark:group-hover:text-slate-300 flex-shrink-0" />
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 border border-slate-200/80 dark:border-slate-800 shadow-2xs">
          ...
          <div className="space-y-2">
            {emergencyVets.map(vet => (
              <div key={vet.id} className="p-2 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/70 dark:border-slate-700 text-xs">
                <p className="font-bold text-slate-800 dark:text-slate-200 truncate">{vet.name}</p>
                <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate mt-0.5">{vet.address}</p>
                <div className="flex items-center justify-between mt-1.5 pt-1 border-t border-slate-200/60 dark:border-slate-700">
                  <span className="text-[10px] font-semibold text-red-600 dark:text-red-400">🚨 Open 24/7</span>
                  <a
                    href={`tel:${vet.phone}`}
                    className="flex items-center gap-1 text-[10px] font-bold text-green-700 dark:text-green-400 bg-white dark:bg-slate-800 px-2 py-0.5 rounded-full border border-slate-200 dark:border-slate-700 hover:bg-green-50 dark:hover:bg-slate-700 transition-colors"
                  >
...
class ErrorBoundary extends (React.Component as new (props: any) => {
  ...
  render() {
    if (this.state?.hasError) {
      return (
        <div className="min-h-screen bg-white dark:bg-[#0b1120] text-slate-800 dark:text-slate-100 flex flex-col items-center justify-center p-4 sm:p-6 text-center font-sans">
...
const MainAppContent: React.FC = () => {
  ...
  if (isAuthLoading) {
    return (
      <div className="min-h-screen bg-white dark:bg-[#0b1120] text-slate-800 dark:text-slate-100 flex flex-col items-center justify-center p-4 font-sans">
        ...
        <div className="flex items-center gap-2 text-slate-700 dark:text-slate-200 font-bold text-sm">
          <Loader2 className="w-4 h-4 animate-spin text-green-700 dark:text-green-400" />
  ...
  if (!user) {
    return (
      <div className="min-h-[100dvh] w-full bg-[#F3F7F4] sm:bg-[#0D1C12] dark:bg-[#070e09] flex flex-col items-center justify-center p-0 sm:p-4 md:p-6 lg:p-8 font-sans transition-colors duration-200">
        <WelcomeScreen />
        <ToastContainer />
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0b1120] text-slate-800 dark:text-slate-100 flex flex-col font-sans antialiased selection:bg-green-600/20 selection:text-green-800 transition-colors duration-200">
      {/* Unified Responsive Header */}
      {!activeAnimalId && <Header />}

      {/* Main Responsive App Content Area */}
      <main className={`${currentTab === 'profile' ? 'w-full' : 'flex-1 w-full'} max-w-7xl mx-auto ${currentTab === 'home' ? 'px-0 sm:px-6 lg:px-8' : 'px-3 sm:px-6 lg:px-8'} py-2.5 sm:py-5 pb-[calc(4.75rem+env(safe-area-inset-bottom,0px))] md:pb-6`}>
        {activeAnimalId ? (
          <AnimalProfileView
      ...
      {!activeAnimalId && (
        <button
          ...
          className="fixed right-4 sm:right-6 bottom-[calc(4.75rem+env(safe-area-inset-bottom,0px))] md:bottom-6 z-30 w-12 h-12 rounded-full bg-gradient-to-tr from-green-800 to-green-600 text-white shadow-lg shadow-green-900/30 flex items-center justify-center text-lg hover:scale-110 active:scale-95 transition-all border-2 border-white dark:border-slate-800 group focus:outline-none focus:ring-2 focus:ring-green-500/50 select-none cursor-pointer"
          title="Ask Pawsy AI"
